...
Revision ID: 365d2b1d07a1
Revises: 
Create Date: 2024-11-09 11:30:37.174477
...
def upgrade():
    ...
    sa.Column('item_model', sa.String(length=200), nullable=False),
    sa.Column('service_type', sa.String(length=200), nullable=False),
    sa.Column('date_brought', sa.DateTime(), nullable=False),
    sa.Column('expected_collection_date', sa.DateTime(), nullable=True),
    sa.Column('amount_due', sa.NUMERIC(precision=10, scale=2), nullable=False),
    sa.Column('progress_status', sa.Enum('PENDING', 'IN_PROGRESS', 'FINAL_STAGES', 'COMPLETED', 'COLLECTED', name='repairstatus'), nullable=False),
    ...
    op.create_index(op.f('ix_repair_services_reference_number'), 'repair_services', ['reference_number'], unique=True)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_repair_services_reference_number'), table_name='repair_services')
